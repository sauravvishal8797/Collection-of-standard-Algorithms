...
int main()
{
    ...
    unsigned long long int pcnt, fpcnt, o, e, a, b, k, num_oc;
    int k1, k2;
    ...
    for (int i=0; i<T; i++)
    {
        ...
        mergeSort(arr[i], e_c[i], N[i]-1);  // Merge sorts the array and returns
        ...
        temp[k1++] = arr[i][e_c[i]-1];
        k2 = k1;

        for (int j=e_c[i]; j<N[i]-1; j++)
        {
            if (arr[i][j] != arr[i][j+1])
            {
                temp[k2++] = arr[i][j];
        ...
        */
        ...
        for (int j=0; j<e_c[i]; j++)
        {
            ...
            if ( temp[j] % 4 == 0 )
            {
                if ( (j <= e_c[i]-2) && ( temp[j]+2 == temp[j+1] ) )
                {
                    //printf("%d\n", temp[j]);
                    k = j+1;
                ...
                /*
                else if ( (j <= e_c[i]-3) && ( temp[j]+2 == temp[j+2] ) )
                {
                    //printf("%d\n", temp[j]);
                    k = j+2;
                    while ( (temp[k] == temp[k+1]) && (k+1 < e_c[i]) )
        ...
        for (int j=e_c[i]; j<N[i]; j++)
        {
            ...
            if ( temp[j] % 4 == 1 )
            {
                ...
                /*
                else if ( (j <= N[i]-3) && ( temp[j]+2 == temp[j+2] ) )
                {
                    //printf("%d\n", temp[j]);
                    k = j+2;
                    while ( (temp[k] == temp[k+1]) && (k+1 < N[i]) )
